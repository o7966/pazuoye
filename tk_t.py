...
def download_one(baseurl, imageurl2, path, i):
    fullurl = "{}{}".format(baseurl,imageurl2)
    r = requests.get(fullurl, headers=headers)
    f = open("{}/{}.jpg".format(path,str(i+1).zfill(3)),'wb')
    f.write(r.content)
...
def pachong():
    ...
    with futures.ThreadPoolExecutor(max_workers = 5) as executor:
        for i, imgurl2 in enumerate(res):
            executor.submit(download_one, baseurl, imgurl2, path.get(), i)
