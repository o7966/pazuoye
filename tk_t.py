...
def download_one(html_data2_url, path, c_num, s_num):
    url1 = 'http://weixin.zijinshe.com/cms/upload/page/' + html_data2_url[2:] + '.jpg'
    """
    ...
    f = open(path + '\\' + str(c_num + 1).zfill(3) + '.jpg', 'wb')
    f.write(r.content)
...
#    url1 = "http://weixin.zijinshe.com/cms/upload/page/"
    ...
    s_num = len(html_data2)
    with futures.ThreadPoolExecutor(max_workers = 5) as executor:
        for c_num, html_data2_url in enumerate(html_data2):
            executor.submit(download_one, html_data2_url, path, c_num, s_num)

    tkinter.messagebox.showinfo('完成', '下载完成！')
